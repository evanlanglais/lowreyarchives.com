...
const allMenus: Array<{
  menu: AwesomeLaoutPageNavbarMenu;
  authenticatedPath: AuthenticatedPathType;
}> = [
  {
    menu: { type: "link", title: "Login", to: { name: "login" } },
    authenticatedPath: AuthenticatedPathType.Unauthenticated,
  },
  {
    menu: { type: "link", title: "Register", to: { name: "register" } },
    authenticatedPath: AuthenticatedPathType.Unauthenticated,
  },
  {
    menu: { type: "link", title: "Home", to: { name: "index" } },
    authenticatedPath: AuthenticatedPathType.Authenticated,
  },
  {
    menu: { type: "link", title: "Archives", to: { name: "archive" } },
    authenticatedPath: AuthenticatedPathType.Authenticated,
  },
  {
    menu: {
  ...
  {
    menu: { type: "button", title: "Logout", to: { name: "logout" } },
    authenticatedPath: AuthenticatedPathType.Authenticated,
  },
];
...
const menus = computed(() => {
  return allMenus
    .filter(
      (item) =>
        item.authenticatedPath === AuthenticatedPathType.Any ||
        (item.authenticatedPath === AuthenticatedPathType.Unauthenticated &&
          !user.value) ||
        (item.authenticatedPath === AuthenticatedPathType.Authenticated &&
          !!user.value),
    )
    .map((item) => item.menu) as AwesomeLayoutPageNavbarMenu[];
});
